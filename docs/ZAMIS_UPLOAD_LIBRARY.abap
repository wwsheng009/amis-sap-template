*&---------------------------------------------------------------------*
*& Report ZAMIS_UPLOAD_LIBRARY
*&---------------------------------------------------------------------*
*& *上传SDK文件到SAP MIME数据库
* 复制程序：/UI5/UI5_REPOSITORY_LOAD
*&---------------------------------------------------------------------*
REPORT zamis_upload_library.

CONSTANTS init_folder TYPE string VALUE 'D:/projects/go/yao/wwsheng009/yao-amis-admin/Public/amis-admin/sdk/'.

SELECTION-SCREEN BEGIN OF BLOCK b1 WITH FRAME TITLE TEXT-001. "Specify SAPUI5 App and Select Operation


* UI5 repository name
* 默认会上传到目录sap/public/bc/ur/amis目录
PARAMETER : ui5rep TYPE string DEFAULT 'amis' OBLIGATORY.
...
CLASS lcx_mime_rep DEFINITION INHERITING FROM cx_static_check.
  PUBLIC SECTION.
...
CLASS lcl_mime_rep DEFINITION.

  PUBLIC SECTION.


    CLASS-METHODS: get_api IMPORTING iv_name            TYPE string
                           RETURNING VALUE(ro_instance) TYPE REF TO lcl_mime_rep.
    ...
    CLASS-METHODS create_repository
      IMPORTING
        ...
        VALUE(iv_transport_request) TYPE trkorr OPTIONAL
        VALUE(iv_devclass)          TYPE devclass OPTIONAL
        iv_dialog_mode              TYPE flag DEFAULT ' '
        iv_no_ato_generated         TYPE abap_bool OPTIONAL
      RETURNING
        VALUE(ro_instance)          TYPE REF TO lcl_mime_rep
      RAISING
        lcx_mime_rep .

    METHODS check_is_folder
      IMPORTING
        iv_path          TYPE string
      RETURNING
    ...
    METHODS get_folder_children
      IMPORTING
        iv_path            TYPE string
      RETURNING
        VALUE(rt_children) TYPE string_table
      RAISING
        lcx_mime_rep .
    ...
    METHODS lock
      IMPORTING
        iv_lock_mode TYPE enqmode DEFAULT /ui5/if_ui5_rep_dt=>c_lock_for_read_write
      RAISING
        lcx_mime_rep .
    ...
    METHODS put_file
      IMPORTING
        ...
        iv_code_page         TYPE string DEFAULT 'UTF-8'
        iv_is_binary         TYPE abap_bool DEFAULT '-'
        iv_skip_virus_scan   TYPE abap_bool DEFAULT abap_false
      EXPORTING
        ev_new_etag          TYPE string
      RAISING
        lcx_mime_rep .
    ...
    METHODS delete
      IMPORTING
        iv_path              TYPE string
        iv_transport_request TYPE trkorr OPTIONAL
        iv_delete_children   TYPE abap_bool DEFAULT abap_false
      RAISING
        lcx_mime_rep .
    ...
    METHODS get_file
      IMPORTING
        iv_path          TYPE string
        iv_code_page     TYPE string DEFAULT 'UTF-8'
        iv_newline_chars TYPE char2 DEFAULT cl_abap_char_utilities=>cr_lf
      EXPORTING
        ev_file_content  TYPE xstring
        ev_mime_type     TYPE csequence
  ...
  PRIVATE SECTION.
    ...
    DATA: l_transport_reques TYPE trkorr,
          l_description      TYPE string,
          l_devclass         TYPE devclass,
          l_dialog_mode      TYPE flag.


ENDCLASS.


CLASS lcl_mime_rep IMPLEMENTATION.
  ...
  METHOD get_api.
    ...
    DATA: l_mr_api TYPE REF TO if_mr_api.

    DATA: l_refix TYPE string.
    CONCATENATE 'SAP/PUBLIC/BC/UR' iv_name INTO l_refix.
*    可以把前缀传进去
    l_mr_api = zcl_amis_mime_repository_api=>if_mr_api~get_api( i_prefix = l_refix ).

    DATA: l_mr_rep TYPE REF TO lcl_mime_rep.
    CREATE OBJECT l_mr_rep.

    l_mr_rep->mr_api = l_mr_api.

    ro_instance = l_mr_rep.
...
*    可以把前缀传进去
  ...
  METHOD check_is_folder.
    CALL METHOD mr_api->properties( EXPORTING i_url = iv_path IMPORTING e_is_folder = rv_result ).
  ENDMETHOD.

  METHOD create_folder.
    mr_api->create_folder( EXPORTING i_url = iv_path ).
  ENDMETHOD.

  METHOD get_folder_children.
    mr_api->file_list( EXPORTING i_url = iv_path IMPORTING  e_files = rt_children ).
  ENDMETHOD.
  ...
  METHOD delete.
    mr_api->delete( EXPORTING i_url = iv_path i_delete_children = abap_true ).
  ENDMETHOD.

  METHOD put_file.
*    由于CL_MIME_REPOSITORY_API中的处理会把文件名自动转换成大小，所以需要自定义一个
*    mr_api->put( EXPORTING i_url = iv_path i_content = iv_file_content   ).
    mr_api->put( EXPORTING i_url = iv_path i_content = iv_file_content  i_description = l_description i_dev_package = l_devclass i_corr_number = l_transport_reques ).
  ...
  METHOD get_file.
    CALL METHOD mr_api->get
      EXPORTING
        i_url              = iv_path
*       i_check_authority  = 'X'
      IMPORTING
*       e_is_folder        =
        e_content          = ev_file_content
...
*       c_language         =
    ...
    IF sy-subrc <> 0.
      ...
      ELSEIF sy-subrc = 4.
        l_message = '没有权限'.
      ENDIF.
      RAISE EXCEPTION TYPE lcx_mime_rep MESSAGE ID '00'
        TYPE 'I'
        NUMBER '001'
        WITH l_message.
...
*    mr_api->get( EXPORTING i_url = iv_path IMPORTING e_content = ev_file_content  ).
  ENDMETHOD.
...
CLASS lcl_ui5_repository DEFINITION FINAL CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    DATA: already_exists                 TYPE abap_bool,
          api                            TYPE REF TO lcl_mime_rep, "/ui5/if_ui5_rep_dt,
          binary_file_identification_msg TYPE string,
...
*    home_directory = file_system->select_directory( iv_title = title iv_initial_folder = 'E:\tmp\test')." 'D:/projects/go/yao/wwsheng009/yao-amis-admin/Public/amis-admin/sdk/' ).
    home_directory = file_system->select_directory( iv_title = title iv_initial_folder = init_folder ).
...
*     Delegate ...
      ...
      TRY.
          ui5_repository->api =
             lcl_mime_rep=>create_repository(
...
*     Determine file type
      ...
      IF me->is_text_file( upload_operation->relative_path ) = abap_true.
        upload_operation->object_type = lcl_operation=>object_type_text_file.
      ENDIF.
      IF me->is_binary_file( upload_operation->relative_path ) = abap_true.
...
*   Check if UI5 Repository already exists. If yes retrieve API.
    TRY.
        me->api = lcl_mime_rep=>get_api( iv_name = me->name ).
